...
boton1.onclick = function () {
    baseDatosLocal = JSON.parse(localStorage.getItem("baseDatosLocal"));
    let cont = [0, 0, 0];
    //0 = Tecnologia, 1 = Alimentos, 2 = Ropa
    baseDatosLocal.forEach(categoriaI => {
        if (categoriaI.categoria == "Tecnologia") {
            cont[0]++;
        } else if (categoriaI.categoria == "Alimentos") {
            cont[1]++;
        } else if (categoriaI.categoria == "Ropa") {
            cont[2]++;
        }
    });
    console.log(cont);
    alert("Tecnologia: " + cont[0] + "   Alimentos: " + cont[1] + "   Ropa: " + cont[2]);
}
...
boton2.onclick = function () {
    baseDatosLocal = JSON.parse(localStorage.getItem("baseDatosLocal"));
    let total = 0;
    baseDatosLocal.forEach(sumador => {
        total = total + parseInt(sumador.cantidad);
    });
...
boton3.onclick = function () {
    ...
    let total = 0;
    baseDatosLocal.forEach(sumador => {
        total = total + parseInt(sumador.valor);
    });
...
function EliminarFilas() {
    ...
    baseDatosLocal.forEach(itera => {
        let item = tabl.querySelector('.tbodys');
